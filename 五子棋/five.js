var fiveChess = {
	...
	clickChess:function (event) {
		...
		if(document.bManage){
			...
			el = $("div.chessboard div").eq(index)
		}
		
		

		if (_fiveChess.chessArr[x][y] === _fiveChess.NO_CHESS && el) {
			_fiveChess.playChess(x, y, _fiveChess.humanPlayer);
			if (x === 0 && y === 0) {
				$(el).removeClass("hover-up-left");
			}
			else if (x === 0 && y === 14) {
				$(el).removeClass("hover-up-right");
			}
			else if (x === 14 && y === 0) {
				$(el).removeClass("hover-down-left");
			}
			else if (x === 14 && y === 14) {
				$(el).removeClass("hover-down-right");
			}
			else if (x === 0) {
				$(el).removeClass("hover-up");
			}
			else if (x === 14) {
				$(el).removeClass("hover-down");
			}
			else if (y === 0) {
			...
			_fiveChess.playerLastChess = [x, y];
			if(document.bManage)_fiveChess.playerWinOrNot(x, y);
		}
	},
	mousein:function(e){
		var _fiveChess = this;
		...
		if (_fiveChess.chessArr[i][j] === _fiveChess.NO_CHESS && el) {
			if (i === 0 && j === 0) {
				$(el).addClass("hover-up-left");
			}
			else if (i === 0 && j === 14) {
				$(el).addClass("hover-up-right");
			}
			else if (i === 14 && j === 0) {
				$(el).addClass("hover-down-left");
			}
			else if (i === 14 && j === 14) {
				$(el).addClass("hover-down-right");
			}
			else if (i === 0) {
				$(el).addClass("hover-up");
			}
			else if (i === 14) {
				$(el).addClass("hover-down");
			}
			else if (j === 0) {
	...
	resetChessBoard: function () {
		...
		if(document.bManage && document.ws){
			document.ws.send(JSON.stringify({
				name:document.name,
				bManage:true,
				msg:'reset'
		...
		var _fiveChess = this;
		if(document.bManage){
			$("div.chessboard div").click(function(e){
				_fiveChess.clickChess(e)
			});
		}

		//鼠标在棋盘上移动效果
		$("div.chessboard div").hover(e=>{
			this.mousein(e)
		},
		e=>{
			this.mouseout(e)
	...
	showResult: function(isPlayerWin) {
		if (isPlayerWin) {
			$("#result_tips").html("你赢了").show();
		}
		else {
			$("#result_tips").html("你输了").show();
		}
